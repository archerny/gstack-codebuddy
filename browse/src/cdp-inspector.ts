...
/** Accepted target: either a Page or a Frame (for iframe context). */
...
/** Map of Page → CDP session. Same-origin frames share the page's session. */
...
/**
 * Resolve the owning Page from a PageLike target.
 * Frames don't have .context() — we walk up to the page.
 */
function resolveOwnerPage(target: PageLike): Page {
  if ('context' in target && typeof target.context === 'function') {
    return target as Page;
  }
  // It's a Frame — get the owning page
  const frame = target as Frame;
  return frame.page();
}

/** Check whether a target is a Page (has .context()) or a Frame. */
function isPage(target: PageLike): target is Page {
  return 'context' in target && typeof target.context === 'function';
}

/**
 * Get or create a CDP session for the given page/frame target.
 * Enables DOM + CSS domains on first use.
 * Frames share the CDP session of their owning page.
 */
async function getOrCreateSession(target: PageLike): Promise<any> {
  const page = resolveOwnerPage(target);
...
/**
 * Resolve the document root nodeId for a given target.
 * For a Page, returns the main document root.
 * For a Frame, walks the DOM tree to find the iframe's contentDocument.
 */
async function resolveDocumentRoot(session: any, target: PageLike): Promise<number> {
  ...
  // For Frame targets, we need to find the iframe element's contentDocument
  // Use the frame's URL or name to locate it
  const frame = target as Frame;
  const frameName = frame.name();
  const frameUrl = frame.url();

  // Get the full document tree to find iframe elements
  const { root: deepRoot } = await session.send('DOM.getDocument', { depth: -1 });

  // Walk the tree to find an iframe/frame element whose contentDocument matches
  const iframeNodeId = findIframeNode(deepRoot, frameName, frameUrl);
  if (iframeNodeId) {
    // Get the contentDocument of the iframe
    const { node } = await session.send('DOM.describeNode', { nodeId: iframeNodeId, depth: 1 });
    if (node.contentDocument) {
      // Request the contentDocument's children so DOM.querySelector works on it
      await session.send('DOM.requestChildNodes', { nodeId: node.contentDocument.nodeId, depth: -1 });
      return node.contentDocument.nodeId;
    }
  }

  // Fallback: try to locate via the frame URL in the frame tree
  // This handles cross-origin iframes that might have been resolved
  return root.nodeId;
}

/**
 * Walk a CDP DOM node tree to find an iframe element matching by name or URL.
 * Returns the nodeId of the matching iframe element, or null.
 */
function findIframeNode(node: any, frameName: string, frameUrl: string): number | null {
  ...
  if (tagName === 'iframe' || tagName === 'frame') {
    ...
    for (let i = 0; i < attrs.length; i += 2) {
      if (attrs[i] === 'name' && attrs[i + 1] === frameName && frameName) {
        return node.nodeId;
      }
    }
    // Check by contentDocument URL
    if (node.contentDocument) {
      const docUrl = node.contentDocument.documentURL || '';
      if (frameUrl && docUrl.includes(frameUrl.replace('about:', ''))) {
        return node.nodeId;
      }
      // For srcdoc iframes, both URLs will be about:srcdoc
      if (frameUrl === 'about:srcdoc' && docUrl === 'about:srcdoc') {
        return node.nodeId;
      }
    }
  }
  ...
  if (node.children) {
    for (const child of node.children) {
      const found = findIframeNode(child, frameName, frameUrl);
      if (found) return found;
    }
  }
  // Also check contentDocument children (for nested iframes)
  if (node.contentDocument?.children) {
 ...
 */
export function computeSpecificity(selector: string): { a: number; b: number; c: number } {
  let a = 0, b = 0, c = 0;
...
export async function inspectElement(
  ...
  const session = await getOrCreateSession(target);

  // Get document root — resolves to iframe contentDocument when target is a Frame
  const rootNodeId = await resolveDocumentRoot(session, target);
...
export async function modifyStyle(
  ...
  try {
    ...
    const session = await getOrCreateSession(target);
    const result = await inspectElement(target, selector);
 ...
 */
export async function undoModification(target: PageLike, index?: number): Promise<void> {
  ...
  if (mod.method === 'setStyleTexts') {
    ...
    try {
      await modifyStyle(target, mod.selector, mod.property, mod.oldValue);
 ...
 */
export async function resetModifications(target: PageLike): Promise<void> {
