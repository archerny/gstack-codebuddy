  ...
  switch (command) {
    ...
    case 'style': {
      ...
      if (args[0] === '--undo') {
        ...
        await undoModification(target, idx);
        return idx !== undefined ? `Reverted modification #${idx}` : 'Reverted last modification';
      }

      // style --reset → revert all modifications
      if (args[0] === '--reset') {
        const count = getModificationHistory().length;
        if (count === 0) return 'No modifications to reset.';
        await resetModifications(target);
        return `Reset all ${count} style modifications`;
      }
      ...
      if (!selector || !property || !value) {
        throw new Error('Usage: browse style <sel> <prop> <value> | style --undo [N] | style --reset');
      }
      ...
      const mod = await modifyStyle(target, selector, property, value);
    ...
    case 'cleanup': {
      ...
      if (selectors.length > 0) {
        const count = await target.evaluate((sels: string[]) => {
          let removed = 0;
      ...
      if (doSticky) {
        const stickyCount = await target.evaluate(() => {
          let removed = 0;
      ...
      const scrollFixed = await target.evaluate(() => {
        let fixed = 0;
      ...
      const adLabelCount = await target.evaluate(() => {
        let removed = 0;
      ...
      // Remove empty ad placeholder whitespace
      const collapsedCount = await target.evaluate(() => {
        let collapsed = 0;
    ...
    case 'prettyscreenshot': {
      ...
      // For prettyscreenshot we need the page-level API for viewport and screenshot
      const page = bm.getPage();
      ...
      if (doCleanup) {
        ...
        await target.evaluate((sels: string[]) => {
          for (const sel of sels) {
            try {
      ...
      if (scrollTo) {
        const scrolled = await target.evaluate((scrollTarget: string) => {
          let el = document.querySelector(scrollTarget);
          if (el) {
          ...
          while ((node = walker.nextNode())) {
            if (node.textContent?.includes(scrollTarget)) {
