...
describe('matchGlob', () => {
  ...
  test('** does not match unrelated paths', () => {
    expect(matchGlob('browse/src/commands.ts', 'skill-templates/qa/**')).toBe(false);
    expect(matchGlob('skill-templates/review/SKILL.md', 'skill-templates/qa/**')).toBe(false);
  });
  ...
  test('** at end matches files in the directory', () => {
    expect(matchGlob('skill-templates/qa/SKILL.md', 'skill-templates/qa/**')).toBe(true);
    expect(matchGlob('skill-templates/qa/SKILL.md.tmpl', 'skill-templates/qa/**')).toBe(true);
    expect(matchGlob('skill-templates/qa/templates/report.md', 'skill-templates/qa/**')).toBe(true);
  });
});
...
describe('selectTests', () => {
  ...
  test('skill-specific change selects only that skill and related tests', () => {
    const result = selectTests(['skill-templates/plan-ceo-review/SKILL.md.tmpl'], E2E_TOUCHFILES);
    expect(result.selected).toContain('plan-ceo-review');
    expect(result.selected).toContain('plan-ceo-review-selective');
    // Also selects journey routing tests (*/SKILL.md.tmpl matches plan-ceo-review/SKILL.md.tmpl)
  ...
  test('multiple changed files union their selections', () => {
    const result = selectTests(
      ['skill-templates/plan-ceo-review/SKILL.md.tmpl', 'skill-templates/retro/SKILL.md.tmpl'],
      E2E_TOUCHFILES,
    );
    expect(result.selected).toContain('plan-ceo-review');
    expect(result.selected).toContain('plan-ceo-review-selective');
  ...
  test('works with LLM_JUDGE_TOUCHFILES', () => {
    const result = selectTests(['skill-templates/qa/SKILL.md.tmpl'], LLM_JUDGE_TOUCHFILES);
  ...
  test('SKILL.md.tmpl root template selects root-dependent tests and routing tests', () => {
    const result = selectTests(['skill-templates/SKILL.md.tmpl'], E2E_TOUCHFILES);
    // skillmd-setup/no-local/outside-git now depend on browse/SKILL.md.tmpl, not root
    expect(result.selected).not.toContain('skillmd-setup-discovery');
    // Root-dependent: contributor-mode, session-awareness
