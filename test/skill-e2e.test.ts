...
function setupBrowseShims(dir: string) {
  ...
  // remote-slug shim (returns test-project) — lives in bin/, not browse/bin/
  const remoteSlugDir = path.join(dir, 'bin');
  fs.mkdirSync(remoteSlugDir, { recursive: true });
  fs.writeFileSync(
    path.join(remoteSlugDir, 'remote-slug'),
    `#!/bin/bash\necho "test-project"\n`,
    { mode: 0o755 },
  );
}
...
Report what each command returned.`,
  ...
  testIfSelected('skillmd-setup-discovery', async () => {
    const skillMd = fs.readFileSync(claudeSkillPath('browse'), 'utf-8');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // ~/.claude/skills/browse/dist/browse exists, so we get READY.
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testIfSelected('contributor-mode', async () => {
    ...
    const skillMd = fs.readFileSync(claudeSkillPath('.'), 'utf-8');
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of the default state directory contributor-logs/
...
describeIfSelected('QA skill E2E', ['qa-quick'], () => {
  ...
  beforeAll(() => {
    ...
    copyDirSync(path.join(ROOT, 'skill-templates', 'qa'), path.join(qaDir, 'qa'));
...
describeIfSelected('Review skill E2E', ['review-sql-injection'], () => {
  ...
  beforeAll(() => {
    ...
    fs.copyFileSync(claudeSkillPath('review'), path.join(reviewDir, 'review-SKILL.md'));
    fs.copyFileSync(path.join(ROOT, 'skill-templates', 'review', 'checklist.md'), path.join(reviewDir, 'review-checklist.md'));
    fs.copyFileSync(path.join(ROOT, 'skill-templates', 'review', 'greptile-triage.md'), path.join(reviewDir, 'review-greptile-triage.md'));
  });
...
describeIfSelected('Review enum completeness E2E', ['review-enum-completeness'], () => {
  ...
  beforeAll(() => {
    ...
    // Copy review skill files
    fs.copyFileSync(claudeSkillPath('review'), path.join(enumDir, 'review-SKILL.md'));
    fs.copyFileSync(path.join(ROOT, 'skill-templates', 'review', 'checklist.md'), path.join(enumDir, 'review-checklist.md'));
    fs.copyFileSync(path.join(ROOT, 'skill-templates', 'review', 'greptile-triage.md'), path.join(enumDir, 'review-greptile-triage.md'));
  });
...
describeE2E('Review design lite E2E', () => {
  ...
  beforeAll(() => {
    ...
    // Copy review skill files
    fs.copyFileSync(claudeSkillPath('review'), path.join(designDir, 'review-SKILL.md'));
    fs.copyFileSync(path.join(ROOT, 'skill-templates', 'review', 'checklist.md'), path.join(designDir, 'review-checklist.md'));
    fs.copyFileSync(path.join(ROOT, 'skill-templates', 'review', 'design-checklist.md'), path.join(designDir, 'review-design-checklist.md'));
    fs.copyFileSync(path.join(ROOT, 'skill-templates', 'review', 'greptile-triage.md'), path.join(designDir, 'review-greptile-triage.md'));
  });
...
(anyOutcomeSelected ? describeOutcome : describe.skip)('Planted-bug outcome evals', () => {
  ...
  beforeAll(() => {
    ...
    // Copy qa skill files
    copyDirSync(path.join(ROOT, 'skill-templates', 'qa'), path.join(outcomeDir, 'qa'));
  });
...
`);
    ...
    fs.copyFileSync(
      claudeSkillPath('plan-ceo-review'),
...
`);
    ...
    fs.copyFileSync(
      claudeSkillPath('plan-eng-review'),
...
describeIfSelected('QA-Only skill E2E', ['qa-only-no-fix'], () => {
  ...
  beforeAll(() => {
    ...
    copyDirSync(path.join(ROOT, 'skill-templates', 'qa-only'), path.join(qaOnlyDir, 'qa-only'));
    ...
    fs.copyFileSync(
      path.join(ROOT, 'skill-templates', 'qa', 'templates', 'qa-report-template.md'),
...
describeIfSelected('QA Fix Loop E2E', ['qa-fix-loop'], () => {
  ...
  beforeAll(() => {
    ...
    // Copy qa skill files
    copyDirSync(path.join(ROOT, 'skill-templates', 'qa'), path.join(qaFixDir, 'qa'));
...
`);
  ...
  test('/plan-eng-review writes test-plan artifact to state directory projects/', async () => {
...
IMPORTANT: After your review, you MUST write the test-plan artifact as described in the "Test Plan Artifact" section of SKILL.md. The remote-slug shim is at ${planDir}/bin/remote-slug.
...
describeIfSelected('Base branch detection', ['review-base-branch', 'ship-base-branch', 'retro-base-branch'], () => {
  ...
  testIfSelected('review-base-branch', async () => {
    ...
    // Copy review skill files
    fs.copyFileSync(claudeSkillPath('review'), path.join(dir, 'review-SKILL.md'));
    fs.copyFileSync(path.join(ROOT, 'skill-templates', 'review', 'checklist.md'), path.join(dir, 'review-checklist.md'));
    fs.copyFileSync(path.join(ROOT, 'skill-templates', 'review', 'greptile-triage.md'), path.join(dir, 'review-greptile-triage.md'));
...
Write your findings to ${dir}/review-output.md`,
  ...
  testIfSelected('ship-base-branch', async () => {
    ...
    // Copy ship skill
    fs.copyFileSync(claudeSkillPath('ship'), path.join(dir, 'ship-SKILL.md'));
...
- The diff stat against the base branch`,
  ...
  testIfSelected('retro-base-branch', async () => {
    ...
    fs.copyFileSync(claudeSkillPath('retro'), path.join(dir, 'retro', 'SKILL.md'));
...
describeIfSelected('Document-Release skill E2E', ['document-release'], () => {
  ...
  beforeAll(() => {
    ...
    copyDirSync(path.join(ROOT, 'skill-templates', 'document-release'), path.join(docReleaseDir, 'document-release'));
...
`);
    ...
    fs.copyFileSync(
      claudeSkillPath('design-consultation'),
...
describeIfSelected('Plan Design Review E2E', ['plan-design-review-plan-mode', 'plan-design-review-no-ui-scope'], () => {
  ...
  beforeAll(() => {
    ...
    fs.copyFileSync(
      claudeSkillPath('plan-design-review'),
...
describeIfSelected('Test Bootstrap E2E', ['qa-bootstrap'], () => {
  ...
  beforeAll(() => {
    ...
    // Copy qa skill files
    copyDirSync(path.join(ROOT, 'skill-templates', 'qa'), path.join(bootstrapDir, 'qa'));
...
describeIfSelected('Test Coverage Audit E2E', ['ship-coverage-audit'], () => {
  ...
  beforeAll(() => {
    ...
    copyDirSync(path.join(ROOT, 'skill-templates', 'ship'), path.join(coverageDir, 'ship'));
    copyDirSync(path.join(ROOT, 'skill-templates', 'review'), path.join(coverageDir, 'review'));
...
describeIfSelected('Codex skill E2E', ['codex-review'], () => {
  ...
  beforeAll(() => {
    ...
    fs.copyFileSync(claudeSkillPath('codex'), path.join(codexDir, 'codex-SKILL.md'));
  });
