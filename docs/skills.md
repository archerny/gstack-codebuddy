...
Both modes end with a design doc written to the gstack state directory — and that doc feeds directly into `/plan-ceo-review` and `/plan-eng-review`. The full lifecycle is now: `office-hours → plan → implement → review → QA → ship → retro`.
...
A **product strategy review** skill.

This skill pushes the agent to think with taste, ambition, user empathy, and a long time horizon. It does not take the request literally. It asks a more important question first:
...
Say you're building a Craigslist-style listing app and the request is:
...
That is what `/plan-ceo-review` does.
...
Visions and decisions are persisted to the gstack state directory so they survive beyond the conversation. Exceptional visions can be promoted to `docs/designs/` in your repo for the team.

---

## `/plan-eng-review`

A **technical architecture review** skill.

Once the product direction is right, a different kind of intelligence is needed. Not more sprawling ideation. Not more "wouldn't it be cool if." The model becomes a rigorous technical lead.
...
`/plan-eng-review` builds the technical spine that carries the product vision.
...
This is where diagrams shine — architecture diagrams, state models, data-flow diagrams, test matrices. Diagrams force hidden assumptions into the open. They make hand-wavy planning much harder.
...
A **plan-phase design audit** skill — reviewing your plan before you write a single line of code.
...
A **design system creation** skill.
...
A **pre-landing code review** skill.

Passing tests do not mean the branch is safe.

`/review` exists because there is a whole class of bugs that can survive CI and still cause production incidents. This skill is not about dreaming bigger or making the plan prettier. It asks:

**What can still break?**

This is a structural audit, not a style nitpick pass. The model looks for things like:
...
No flattery. The model imagines the production incident before it happens.
...
A **browser-based QA testing** skill.
...
A **release engineering** skill.

Once the direction is decided, the technical plan is nailed, and review is done, it's time for execution.

`/ship` is for the final mile — for a ready branch, not for deciding what to build.

The model stops behaving like a brainstorm partner and starts behaving like a disciplined release engineer: sync with main, run the right tests, make sure the branch state is sane, update changelog or versioning if the repo expects it, push, and create or update the PR.
...
A **weekly retrospective** skill.

At the end of the week, this skill analyzes what actually happened — not vibes, but data. `/retro` analyzes commit history, work patterns, and shipping velocity and writes a candid retrospective.
...
A **browser session import** skill.
...
A **cross-model second opinion** skill.
...
[Greptile](https://greptile.com) is a service that reviews PRs automatically. It catches real bugs — race conditions, security issues, things that pass CI and break in production.
...
Every false positive you confirm gets saved to the state directory's `greptile-history.md`. Future runs auto-skip known FP patterns for your codebase. And `/retro` tracks Greptile's batting average over time — so you can see whether the signal-to-noise ratio is improving.
