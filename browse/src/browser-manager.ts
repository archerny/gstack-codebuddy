...
import { chromium, type Browser, type BrowserContext, type BrowserContextOptions, type Page, type Locator, type Cookie, type Frame } from 'playwright';
...
export class BrowserManager {
  ...
  /**
   * Return the active frame if set, otherwise the active page.
   * Commands that operate on page content should use this instead of getPage()
   * to support iframe context switching via the `frame` command.
   */
  ...
  /**
   * Close all open pages. Used by state load to replace (not merge) pages.
   */
  ...
  /** Get the ref map for external access (e.g., server inspector routes). */
