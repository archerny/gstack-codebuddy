...
## Execution model: single agent, skill switching

**gstack is a toolbox, not an orchestration framework.** Understanding this distinction is critical to setting correct expectations.

### What gstack is

A single AI agent loads one skill at a time. Each skill is a SKILL.md file — a structured prompt that gives the agent a specialist persona and a step-by-step SOP (Standard Operating Procedure). The user (or the agent's suggestion) triggers a skill switch by name. After the switch, the agent follows the new skill's SOP until the task is done or the user switches again.

```
User ──→ AI Agent ──→ loads SKILL.md ──→ follows SOP ──→ done
              ↑                                              │
              └──────── user switches skill ←────────────────┘
```

This is the same architecture on **all supported platforms** (Claude Code, Codex, CodeBuddy). The differences between platforms are in SKILL.md formatting and platform-specific features (e.g., Claude's `hooks:` frontmatter for enforced safety constraints), not in the execution model.

### What gstack is NOT

- **Not a multi-agent system.** There is no agent-to-agent delegation, no sub-agent spawning, no parallel agent coordination. One agent, one skill, one task at a time.
- **Not an automated pipeline.** Skills do not chain automatically. `/review` does not auto-trigger `/qa`, `/qa` does not auto-trigger `/ship`. Each transition requires an explicit human decision (or agent suggestion that the human accepts).
- **Not an orchestrator.** gstack does not schedule, dispatch, or coordinate work across multiple AI sessions. Tools like [Conductor](https://conductor.build) handle multi-session parallelism — gstack provides the skills each session runs, but doesn't manage the sessions themselves.

### The sprint analogy — and its limits

The README describes skills as following a sprint: "Think → Plan → Build → Review → Test → Ship → Reflect." This is the *recommended workflow order*, not an automatic pipeline. Each arrow is a **manual gear shift** — you decide when to move to the next stage. The agent may suggest "you should run `/qa` next" but it won't do it for you.

### Why not orchestration?

Deliberate design choice. Orchestration adds complexity (error recovery, state management, rollback) and removes human judgment from transitions that benefit from it. The moment between "review done" and "start QA" is often where the most important decisions happen — scope adjustments, priority changes, "actually let me rethink this." Automating that away optimizes for speed at the cost of quality.

### Platform-specific nuances

| Platform | Skill discovery | Invocation | Unique capabilities |
|----------|----------------|------------|---------------------|
| **Claude Code** | `.claude/skills/` directory | `/skill-name` slash commands | `hooks:` frontmatter (enforced safety constraints), `/codex` cross-model collaboration |
| **Codex** | `.agents/skills/` directory | `/skill-name` or natural language | Minimal frontmatter |
| **CodeBuddy** | `.codebuddy/skills/` directory | AI auto-matches based on `description` field | Custom slash commands (separate system from skills) |

On CodeBuddy, skill invocation is **description-driven**: the AI reads the `description` in SKILL.md's YAML frontmatter and loads the matching skill when the user's request aligns. The `/skill-name` references in skill descriptions serve as suggested phrasings — the user can type them, and the AI recognizes them as triggers, but they're not native platform slash commands.
...
| `{{PREAMBLE}}` | `gen-skill-docs.ts` | Startup block: session tracking, proactive check, contributor mode, AskUserQuestion format |
...
Every skill starts with a `{{PREAMBLE}}` block that runs before the skill's own logic. It handles three things in a single bash command:

1. **Session tracking** — touches `$_STATE_DIR/sessions/$PPID` and counts active sessions (files modified in the last 2 hours). When 3+ sessions are running, all skills enter "ELI16 mode" — every question re-grounds the user on context because they're juggling windows.
2. **Contributor mode** — reads `gstack_contributor` from config. When true, the agent files casual field reports to `$_STATE_DIR/contributor-logs/` when gstack itself misbehaves.
