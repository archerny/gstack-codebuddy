...
export function generateBrowseSetup(ctx: TemplateContext): string {
  ...
  // All hosts use $_GSTACK_ROOT probe and derive browse skill root
  return `## SETUP (run this check BEFORE any browse command)

\`\`\`bash
${rootDetect}# Derive browse skill root from gstack root
_BROWSE_ROOT=""
[ -n "$_GSTACK_ROOT" ] && _BROWSE_ROOT="$(dirname "$_GSTACK_ROOT")/browse"
B=""
[ -n "$_BROWSE_ROOT" ] && [ -x "$_BROWSE_ROOT/dist/browse" ] && B="$_BROWSE_ROOT/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
