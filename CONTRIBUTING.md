...
a report to the state directory's `contributor-logs/` with what happened, repro steps, and what
...
2. **Check your logs:** `ls .gstack/contributor-logs/` (project install) or `ls ~/.gstack/contributor-logs/` (global install)
...
├── skill-templates/             <- all skill templates live here
│   ├── review/
│   │   └── SKILL.md             <- edit the .tmpl, test with /review
│   ├── ship/
│   │   └── SKILL.md
│   └── ...
├── browse/
...
```bash
# 1. Edit the template
vim skill-templates/SKILL.md.tmpl              # or browse/SKILL.md.tmpl
...
## Multi-host development (Claude + Codex + CodeBuddy)

gstack generates SKILL.md files for three hosts: **Claude** (`.claude/skills/`), **Codex** (`.agents/skills/`), and **CodeBuddy** (`.codebuddy/skills/`). Every template change needs to be generated for all three.
...
| Aspect | Claude | Codex | CodeBuddy |
|--------|--------|-------|-----------|
| Output directory | `{skill}/SKILL.md` | `.agents/skills/{skill}/SKILL.md` | `dist/codebuddy/{skill}/SKILL.md` |
| Frontmatter | Full (name, description, allowed-tools, hooks, version) | Minimal (name + description only) | None |
| Paths | `~/.claude/skills/gstack` | `~/.codex/skills/gstack` | `$_GSTACK_ROOT` (runtime detection) |
| Hook skills | `hooks:` frontmatter (enforced by Claude) | Inline safety advisory prose (advisory only) | N/A |
| `/codex` skill | Included (Claude wraps codex exec) | Excluded (self-referential) | N/A |
| Path resolution | Hardcoded absolute paths | Hardcoded absolute paths | Runtime `$_GSTACK_ROOT` detection chain |

### Testing output

```bash
# Run all static tests (includes all hosts validation)
bun test

# Check freshness for all hosts
bun run gen:skill-docs --dry-run
bun run gen:skill-docs --host codex --dry-run
bun run gen:skill-docs --host codebuddy --dry-run

# Health dashboard covers all hosts
bun run skill:check
```

### Dev setup for .agents/ and .codebuddy/

When you run `bin/dev-setup`, it creates symlinks in `.claude/skills/`, `.agents/skills/`, and `.codebuddy/skills/` (if applicable), so all host agents can discover your dev skills.

### Adding a new skill

When you add a new skill template, both hosts get it automatically:
1. Create `skill-templates/{skill}/SKILL.md.tmpl`
2. Run `bun run gen:skill-docs` (Claude output), `bun run gen:skill-docs --host codex` (Codex output), and `bun run gen:skill-docs --host codebuddy` (CodeBuddy output)
3. The dynamic template discovery picks it up — no static list to update
4. Commit both `skill-templates/{skill}/SKILL.md.tmpl` and the generated dist files
...
### Alternative: reinstall from a different branch

If you don't want per-project symlinks, you can reinstall from a specific branch:

```bash
cd /path/to/your/gstack-checkout
git fetch origin && git checkout origin/<branch>
bun install && bun run build
./setup              # re-installs to ~/.claude/skills/ (physical copy)
```
