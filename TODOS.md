...
**Current workaround:** `copyRuntimeAssets()` copies `browse/src/` to `dist/{host}/browse/src/` so the compiled binary can find `server.ts` at `../src/server.ts` relative to `browse/dist/browse`. This works but adds ~140KB of source files to each host's dist. The root fix is to bundle server.ts into the binary.
...
### ~~Design docs → Supabase team store sync~~ — CANCELLED

❌ 不做。远端数据上传已移除，所有分析数据仅保留在本地。
