...
function resolveStateDir(): string {
  ...
  if (process.env.GSTACK_STATE_DIR) return process.env.GSTACK_STATE_DIR;

  try {
    const projectRoot = execSync('git rev-parse --show-toplevel 2>/dev/null', { encoding: 'utf-8' }).trim();
    if (projectRoot) {
      // Check if gstack is installed locally (project install)
      // 1. Check dist/*/gstack/bin (self-contained install)
      const distDir = path.join(projectRoot, 'dist');
      if (fs.existsSync(distDir)) {
        try {
          for (const entry of fs.readdirSync(distDir)) {
            if (fs.existsSync(path.join(distDir, entry, 'gstack', 'bin'))) {
              return path.join(projectRoot, '.gstack');
            }
          }
        } catch { /* ignore */ }
      }
      // 2. Check host-specific skill directories
      for (const dir of ['.claude/skills/gstack', '.codebuddy/skills/gstack', '.agents/skills/gstack']) {
        if (fs.existsSync(path.join(projectRoot, dir, 'bin'))) {
          return path.join(projectRoot, '.gstack');
        }
      }
    }
...
const ANALYTICS_FILE = path.join(resolveStateDir(), 'analytics', 'skill-usage.jsonl');
