...
## [0.14.0] - 2026-04-03 — Upstream Sync: 9 New Skills, Cross-Session Memory, and a Smarter Browser

**gstack now has 27 skills, remembers what it learns across sessions, and can inspect CSS like a browser DevTools.**

This release syncs gstack-codebuddy with upstream gstack v0.14.3 — the largest update since the fork. Every change was adapted for CodeBuddy: paths, branding, frontmatter, and platform-specific features all work natively.

### 9 new skills

- **`/learn`** — Record and search learnings across sessions. Six commands: show recent, search, prune, export, stats, manual add. Your institutional memory grows with every session.
- **`/autoplan`** — One command runs CEO review → design review → engineering review in sequence. Automatically chains `/office-hours`, `/plan-ceo-review`, `/plan-design-review`, and `/plan-eng-review` with smart skip logic.
- **`/cso`** — Security audit covering OWASP Top 10, supply chain risks, secrets detection, and infrastructure review. Includes an adversarial step where a second AI challenges the findings.
- **`/setup-deploy`** — Detects your deployment platform (Fly, Render, Vercel, Netlify, Heroku, Railway) and walks you through configuration. Saves the setup to your project docs so you never configure twice.
- **`/benchmark`** — Performance regression detection. Captures page load times, resource sizes, and performance budgets, then compares against a baseline to catch slowdowns before they ship.
- **`/canary`** — Monitors canary deployments by comparing pre-deploy and post-deploy browser snapshots, console errors, and page health. Alerts on regressions.
- **`/land-and-deploy`** — Full merge-to-production workflow: pre-merge checks, CI wait, merge PR, deploy, canary verification, and automatic revert if something breaks. 10 steps, one command.
- **`/design-shotgun`** — Generates multiple design variants in parallel, shows a comparison view, and lets you pick the best one. Requires the design CLI tool.
- **`/design-html`** — Turns design specs into production HTML/CSS. Reads your DESIGN.md constraints and generates code that matches. Requires the design CLI tool.

### Cross-session memory (Learnings system)

- **Every session builds on the last.** `/review`, `/ship`, `/investigate`, `/office-hours`, `/plan-ceo-review`, and `/plan-eng-review` now search previous learnings before starting — and record new findings when they finish.
- **Search across projects.** `gstack-learnings-search --cross-project` finds relevant learnings from your other repositories.
- **Confidence decay.** Older learnings are weighted less than recent ones, so stale knowledge doesn't dominate.

### Smarter reviews and shipping

- **Adversarial review is always on.** Every `/review` now includes an adversarial step where a second AI perspective challenges the findings. Replaces the old hardcoded Codex-only second opinion.
- **Scope drift detection.** `/review` and `/ship` now detect when your changes have drifted beyond the original plan and flag the drift.
- **Confidence scoring.** Review findings now carry 1-10 confidence scores. High-confidence findings (7+) show normally; low-confidence ones (<5) are suppressed to reduce noise.
- **Test failure triage.** `/ship` now distinguishes between failures caused by your changes and pre-existing failures, so you don't waste time debugging someone else's broken test.
- **Plan completion audit.** `/ship` and `/review` check whether your implementation actually covers what the plan promised.
- **CHANGELOG automation.** `/ship` now generates CHANGELOG entries automatically from your diff and commit history.
- **GitLab support.** `/ship` and `/review` now detect GitLab merge requests alongside GitHub PRs.

### Office hours upgrades

- **Anti-sycophancy rules.** `/office-hours` now pushes back on weak ideas instead of agreeing with everything. Five "never say" rules and two "always do" rules keep the conversation honest.
- **Landscape awareness.** Before brainstorming, `/office-hours` researches your competitive landscape via web search (with a privacy gate) and synthesizes what's out there.
- **34 founder resources.** Curated videos, podcasts, and articles surfaced at the right moments during the session.
- **Design mockup and sketch.** `/office-hours` can now generate design sketches and mockups as part of the brainstorming flow.

### Browser: CSS Inspector and new commands

- **`inspect` command** — Full CSS cascade inspection powered by Chrome DevTools Protocol. See computed styles, specificity, box model, and which rules are overridden — just like browser DevTools.
- **`style` command** — Modify CSS properties on live elements. Change colors, fonts, spacing without touching source code.
- **`cleanup` command** — Remove cookie banners, ad overlays, sticky headers, and social share widgets from the page for clean screenshots.
- **`prettyscreenshot` command** — Enhanced screenshots with automatic cleanup and formatting.
- **`frame` command** — Navigate into iframes for cross-frame inspection and interaction.
- **Cross-platform support.** New `platform.ts` abstractions and `bun-polyfill.cjs` for Windows compatibility.

### Resolver architecture

- **36 template resolvers** organized into 13 module files (was 10 resolvers in a single 1920-line file). Every shared behavior — confidence scoring, adversarial review, test triage, scope drift — is now a reusable resolver that multiple skills compose.
- **Preamble tiers (T1-T4).** Skills get different levels of preamble content based on their complexity tier. Lightweight skills like `/browse` get T1 (minimal); heavyweight skills like `/ship` get T4 (full audit capabilities).
- **Parameterized placeholders.** Template syntax now supports `{{NAME:arg1:arg2}}` for composable skill injection.

### For contributors

- `scripts/resolvers/` directory with 13 module files: types.ts, constants.ts, index.ts, preamble.ts, browse.ts, design.ts, review.ts, testing.ts, utility.ts, confidence.ts, composition.ts, codex-helpers.ts, learnings.ts
- `gen-skill-docs.ts` reduced from ~1920 to ~625 lines
- `SKILL_TIER_MAP` in gen-skill-docs.ts maps 26 skills to T1-T4 tiers
- 88 generated SKILL.md files across 3 hosts (30 Claude + 29 CodeBuddy + 29 Codex)
- 3 new bin scripts: `gstack-learnings-log`, `gstack-learnings-search`, `gstack-repo-mode`
- `ETHOS.md` — Search Before Building framework (three layers: codebase search, world search, eureka moments)
- 1,487 tests passing, 0 failures
- Full migration documented in `refactoring/upstream-sync-01-v0.14.3.md`
